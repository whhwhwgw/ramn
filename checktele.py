...
@fifthon.on(events.NewMessage(outgoing=True, pattern=r"\.اوامر الصيد"))
async def _(event):
    if ispay2[0] == "yes":
...
●━━━━━━━●
    ...
    {str(eee)}''')
                    if "A wait of" in str(eee):
                        ...
                        await fifthon.send_message(event.chat.id, " اجاك يوزر متاح  !")
            else:
                pass
            trys += 1
        ...
        isclaim.append("off")
        trys = ""
        await event.client.send_message(event.chat_id, "! وقف الصيد")
